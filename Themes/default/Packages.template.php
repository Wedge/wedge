...
function template_control_chmod()
{
	...
	function testFTP()
	{
		...
		// Post the data out.
		$.post(weUrl(\'action=admin;area=packages;sa=ftptest;' . $context['session_query'] . '\'), sPostData, testFTPResults);
	}
...
function template_file_permissions()
{
	...
	function expandFolder(folderIdent, folderReal)
	{
		...
		show_ajax();
		$.get(weUrl("action=admin;area=packages;onlyfind=" + encodeURIComponent(folderReal) + ";sa=perms;', $context['session_query'], '"), onNewFolderReceived);
	...
	function dynamicAddMore()
	{
		...
		$.get(weUrl("action=admin;area=packages;fileoffset=" + (parseInt(this.offset) + ', $context['file_limit'], ') + ";onlyfind=" + this.path.php_urlencode() + ";sa=perms;', $context['session_query'], '"), onNewFolderReceived);
	}
