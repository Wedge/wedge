...
function aeva_build_object($input)
{
	...
	$arr =& $sites[$upto];
	$use_object_init = we::$is_ajax || WEDGE == 'SSI' || (isset($_REQUEST['action']) && $_REQUEST['action'] == 'feed') || !empty($settings['embed_noscript']) || !empty($context['embed_mg_hack']);
	...
	if (!empty($plugin) && (we::$is_ajax || WEDGE == 'SSI'))
	{
		if ($swfobjects++ === 0)
