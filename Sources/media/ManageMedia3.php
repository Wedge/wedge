...
function aeva_admin_embed()
{
	...
	if (!isset($modSettings['media_lookup_result']) || ((time() - 24*3600) >= @$modSettings['media_lookup_test']) || isset($_GET['flt']))
	{
		...
		$modSettings['media_lookup_result'] = $test = empty($data) || strlen($data) < 50 ? 0 : 1;

		// Save the result so we don't need to run this again.
		$results = array('media_lookup_test' => time(), 'media_lookup_result' => $test);
		if (!isset($modSettings['media_lookups']))
			$results['media_lookups'] = (int) $modSettings['media_lookup_result'];
		updateSettings($results);

		$test = $txt['media_lookup_' . (empty($test) ? 'fail' : 'success')];
	}
	else
	...
	$settings = array(
		'autoembed'			=> array('yesno', 'config'),
		'media_admin_labels_embed' => array('title', 'config'),
		'hr1'					=> array('hr', 'config'),
		'media_lookups'			=> array('yesno', 'config', 'subtext' => $test, 'disabled' => !empty($modSettings['media_lookup_result']) ? 0 : 1),
		'media_yq'				=> array('select', 'config', array(&$txt['media_yq_default'], &$txt['media_yq_hd'])),
		'hr2'					=> array('hr', 'config'),
		'media_titles'			=> array('select', 'config', array(&$txt['media_titles_yes'], &$txt['media_titles_yes2'], &$txt['media_titles_no'], &$txt['media_titles_no2'])),
		'media_lookup_titles'	=> array('yesno', 'config'),
		'media_inlinetitles'		=> array('select', 'config', array(&$txt['media_inlinetitles_yes'], &$txt['media_inlinetitles_maybe'], &$txt['media_inlinetitles_no'])),
		'hr3'					=> array('hr', 'config'),
		'media_center'			=> array('yesno', 'config'),
		'media_incontext'		=> array('yesno', 'config'),
		'media_quotes'			=> array('yesno', 'config'),
		'media_fix_html'			=> array('yesno', 'config'),
		'media_includeurl'		=> array('yesno', 'config'),
		'hr4'					=> array('hr', 'config'),
		'media_noscript'			=> array('yesno', 'config'),
		'media_expins'			=> array('yesno', 'config'),
		'media_debug'			=> array('yesno', 'config'),
		'media_checkadmin'		=> array('yesno', 'config'),
		'hr5'					=> array('hr', 'config'),
		'media_max_width'		=> array('small_text', 'config', null, null, $txt['media_pixels']),
		'media_max_per_post'		=> array('small_text', 'config', null, null, $txt['media_lower_items']),
		'media_max_per_page'		=> array('small_text', 'config', null, null, $txt['media_lower_items']),
		'media_local'			=> array('title', 'config'),
		'media_nonlocal'			=> array('yesno', 'config'),
		'media_ext'				=> array('checkbox_line', 'config', array(), true),
	);

	foreach (array('mp3','mp4','flv','avi','divx','mov','wmp','real','swf') as $ext)
		$settings['media_ext'][2]['media_' . $ext] = array($txt['media_' . $ext], !empty($modSettings['media_' . $ext]), 'force_name' => 'media_' . $ext);
	...
	if (isset($_POST['submit_aeva']))
	{
		...
		if (!$is_sites)
		{
			...
			if (isset($_POST['media_max_per_page']))
			{
				$_POST['media_max_per_page'] = min(1000, max(1, (int) $_POST['media_max_per_page']));
				$_POST['media_max_per_post'] = min(min(1000, max(1, (int) $_POST['media_max_per_post'])), $_POST['media_max_per_page']);
			}
	...
	if ($is_sites)
		$warning_message =
			'<span style="font-weight: normal; color: ' . (empty($modSettings['media_lookup_result']) ? 'red' : 'green') .
			'" class="smalltext">' . $txt['media_' . (empty($modSettings['media_lookup_result']) ? 'fish' : 'denotes')] . '</span>';
	...
	if (empty($modSettings['autoembed']))
	{
		$settings = array(
			...
			'autoembed' => array('yesno', $context['current_area']),
		);
	}
...
function aeva_update_sitelist()
{
	...
	updateSettings(array('media_latest_version' => $latest_version, 'media_latest_sitelist' => $latest_sitelist, 'media_version_test' => time()));
	...
	$context['media_auto_updating'] = true;
	foreach ($modSettings as $ms => $cms)
		if (substr($ms, 0, 6) == 'media_' && !empty($cms))
			$_POST[$ms] = $cms;
	unset($_REQUEST['sa']);
	aeva_admin_embed();
	$_POST = $ps;
...
function aeva_prepare_sites(&$original_array, $type, $is_sites, &$checkall)
{
	...
	if ($is_sites && $type != 'local' && (empty($_POST['media_'.$type]) || !is_array($_POST['media_'.$type])))
	{
		$checkall = false;
		return array();
	}
	...
	if ($type != 'local' && ($is_sites ? empty($modSettings['media_lookups']) : empty($_POST['media_lookups'])))
	...
	if ($type != 'local' && ($is_sites ? empty($modSettings['media_fix_html']) : empty($_POST['media_fix_html'])))
	...
	foreach ($array as $a => $b)
	{
		if ($type == 'local')
		{
			...
			elseif ($is_sites ? empty($modSettings['media_' . substr($b['id'], 6)]) : !isset($_POST['media_' . substr($b['id'], 6)]))
				unset($array[$a], $b);
		}
		// Site disabled? Skip it
		elseif ($is_sites ? !isset($_POST['media_' . $type][$b['id']]) : $b['disabled'])
			unset($array[$a], $b);
		...
		if (isset($array[$a]['lookup-title']) && ($is_sites ? !empty($modSettings['media_titles']) : !empty($_POST['media_titles']))
		&& (empty($array[$a]['lookup-title-skip']) || (!empty($modSettings['media_titles']) && ($modSettings['media_titles'] % 2 == 1))))
			unset($array[$a]['lookup-title']);

		$checkall &= !($original_array[$a]['disabled'] = empty($array[$a]));
	}
	unset($_POST['media_' . $type]);
...
function aeva_settings(&$dest, &$array, $type, $checkall)
{
	...
	$dest['media_' . $type] = array('title', 'sites', null, null, null, 'force_title' => '<strong>' . $txt['media_' . $type . '_sites'] . ' (' . count($array) . ')</strong> - <input type="checkbox" id="checkall_' . $type . '" onclick="invertAll(this, this.form, \'media_' . $type . '\');" ' . (!empty($checkall[$type]) ? ' checked' : '') . '><label class="smalltext" for="checkall_' . $type . '"> <em>' . $txt['media_select'] . '</em></label>');
	$dest['media_' . $type . '_items'] = array('checkbox_line', 'sites', array(), true, null, 'skip_left' => true);
	...
	foreach ($array as $arr)
	{
		$link = (!empty($arr['website']) ? '<a href="' . $arr['website'] . '" style="text-decoration: none" title="-" target="_blank">&oplus;</a> ' : '') . $arr['title']
				. (!empty($arr['lookup-url']) ? '<span style="color: ' . (empty($modSettings['media_lookup_result']) ? 'red' : 'green') . '">*</span>' : '');
		$dest['media_' . $type . '_items'][2]['media_' . $arr['id']] = array($link, !$arr['disabled'], 'force_name' => 'media_' . $type . '[' . $arr['id'] . ']');
	}
}
