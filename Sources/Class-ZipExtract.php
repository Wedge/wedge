...
class wextr
{
	...
	public function __construct($zipname)
	{
		if (!is_callable('gzopen'))
			throw new wextr_NoGzip_Exception();
	...
	public function list_contents($refresh = false)
	{
		...
		if (@fseek($this->zip_fd, $ecd['starting_offset']))
			throw new wextr_InvalidSize_Exception(); // Remember, fseek returns non-zero for error. Bizarrely.
	...
	public function extractByIndex($indexes)
	{
		...
		foreach ($this->_content as $id => $entry)
		{
			...
			if ($entry['compression'] != 8 && $entry['compression'] != 0)
				throw new wextr_UnsupportedCompress_Exception($entry['filename']);
			// Is it encrypted?
			if ($entry['flag'] & 1 == 1)
				throw new wextr_Encrypted_Exception($entry['filename']);
			...
			if (@fseek($this->zip_fd, $entry['offset']))
				throw new wextr_InvalidZip_Exception($entry['filename']);
			...
			if (strlen($new_indexes[$id]['content']) != $entry['uncompressed_size'])
				throw new wextr_InvalidExtraction_Exception($entry['filename']);
		}
		$this->closeFile();
	...
	private function _findFiles($files_to_match, $regex)
	{
		...
		if (empty($this->_content))
			throw new wextr_InvalidZip_Exception();
	...
	public function openFile()
	{
		if ($this->zip_fd != 0 || !($this->zip_fd = @fopen($this->zipname, 'rb')))
			throw new wextr_UnableRead_Exception($this->zipname);
	}
	...
	private function _getEndCentralDir()
	{
		...
		if ($ecd_pos != $file_size - 22)
			throw new wextr_UnableRead_Exception($this->zipname); // in case for some reason we seeked but did not find
		...
		if (strlen($header) != 18)
			throw new wextr_InvalidRecord_Exception(strlen($header));
	...
	private function _getNextCentralDirEntry()
	{
		...
		if (strlen($block) != 46)
			throw new wextr_InvalidBlock_Exception();
	...
	private function _getNextFileEntry()
	{
		...
		if (strlen($block) != 30)
			throw new wextr_InvalidRecord_Exception();
		...
		$data['filename'] = wextr::reduce_path($data['filename']);
		$data['timestamp'] = !empty($data['mdate']) ? $this->_convert_dos_to_timestamp($data['mdate'], $data['mtime']) : time();
	...
	{
		$temp_name = $this->_opts['temp_dir'] . uniqid('wextr-') . '.tmp';
		$temp_fd = $use_gzip ? @gzopen($temp_name, 'wb') : @fopen($temp_name, 'wb');
		if (empty($temp_fd))
			throw new wextr_UnableWrite_Exception($temp_name);
 ...
 * This also removes any need for wextr to care about translations or error messages and such like.
 */
class wextr_NoGzip_Exception extends Exception {}
class wextr_UnableRead_Exception extends Exception {}
class wextr_InvalidExtraction_Exception extends Exception {}
class wextr_InvalidSize_Exception extends Exception {}
class wextr_InvalidZip_Exception extends Exception {}
class wextr_InvalidBlock_Exception extends Exception {}
