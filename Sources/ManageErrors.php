	...
	for ($i = 0; $row = weDB::fetch_assoc($request); $i ++)
	{
		...
		if (!empty($row['file']) && !empty($row['line']))
		{
			...
			$context['errors'][$row['id_error']]['file'] = array(
				...
				'link' => $linkfile ? '<a href="' . $scripturl . '?action=admin;area=logs;sa=errorlog;file=' . base64_encode($row['file']) . ';line=' . $row['line'] . '" data-onclick="return reqWin(this, 600, 400, false);">' . $row['file'] . '</a>' : $row['file'],
