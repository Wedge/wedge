...
/*	This file had one very clear purpose.  It is here expressly to find and
	retrieve information about recently posted topics that we've posted in before.
...
function Unreadreplies()
{
	...
	if (!empty($have_temp_table))
	{
		...
		list ($num_topics) = wedb::fetch_row($request);
		wedb::free_result($request);
	}
	else
	{
	...
	while ($row = wedb::fetch_assoc($request))
	{
		...
		if (!empty($settings['message_index_preview']))
		{
			...
			if (westring::strlen($row['first_body']) > 128)
				$row['first_body'] = westring::substr($row['first_body'], 0, 128) . '...';
			$row['last_body'] = strip_tags(strtr(parse_bbc($row['last_body'], $row['last_smileys'], $row['id_last_msg']), array('<br />' => '&#10;')));
			if (westring::strlen($row['last_body']) > 128)
				$row['last_body'] = westring::substr($row['last_body'], 0, 128) . '...';
