...
function list_getGroupCount()
{
	...
	list ($group_count) = wesql::fetch_row($request);
	wesql::free_result($request);
...
function MembergroupMembers()
{
	...
	if (wesql::num_rows($request) == 0)
		fatal_lang_error('membergroup_does_not_exist', false);
	$context['group'] = wesql::fetch_assoc($request);
	wesql::free_result($request);
	...
	elseif (isset($_REQUEST['add']) && (!empty($_REQUEST['toAdd']) || !empty($_REQUEST['member_add'])) && $context['group']['assignable'])
	{
		...
		$_REQUEST['toAdd'] = strtr(westr::htmlspecialchars($_REQUEST['toAdd'], ENT_QUOTES), array('&quot;' => '"'));
		...
		{
			$member_names[$index] = trim(westr::strtolower($member_names[$index]));
	...
	list ($context['total_members']) = wesql::fetch_row($request);
	wesql::free_result($request);
...
function list_getGroupRequestCount($where, $where_parameters)
{
	...
	list ($totalRequests) = wesql::fetch_row($request);
	wesql::free_result($request);
