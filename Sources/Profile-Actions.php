...
function issueWarning($memID)
{
		...
		if (!empty($_POST['warn_notify']) && empty($issueErrors))
		{
			...
			else
			{
				...
				wesql::insert('',
					...
					array(
						westr::htmlspecialchars($_POST['warn_sub']), westr::htmlspecialchars($_POST['warn_body']),
					),
					array('id_notice')
				);
				$id_notice = wesql::insert_id();
			}
		}
	...
	if (isset($_REQUEST['msg']) && 0 < (int) $_REQUEST['msg'])
	{
		...
		if (wesql::num_rows($request) != 0)
		{
			...
			list ($context['warned_message_subject']) = wesql::fetch_row($request);
		}
		wesql::free_result($request);
