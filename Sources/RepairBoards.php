 ...
 * Wedge
 *
 * Processes the various tables for the "Find/Repair" maintenance option.
 *
 * @package wedge
...
function RepairBoards()
{
	...
	else
	{
		...
		call_hook('repair_errors_finished');
...
function pauseRepairProcess($to_fix, $current_step_description, $max_substep = 0, $force = false)
{
	...
	$context['continue_get_data'] = '?action=admin;area=repairboards' . (isset($_GET['fixErrors']) ? ';fixErrors' : '') . ';step=' . $_GET['step'] . ';substep=' . $_GET['substep'] . ';' . $context['session_query'];
...
function loadForumTests()
{
	...
	*/
	...
	call_hook('repair_errors_tests', array(&$errorTests));
}
