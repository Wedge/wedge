	...
	foreach ($actions as $action)
	{
		...
		elseif ($action['type'] == 'readme')
		{
			...
			if (!empty($action['parse_bbc']))
			{
				...
				wedit::preparsecode($context['package_readme']);
...
function PackageInstall()
{
	...
	loadSubTemplate('extract_package');
	...
	if (!empty($db_changes) && !empty($_POST['do_db_changes']))
	{
		...
		foreach ($db_changes as $change)
		{
			if ($change[0] == 'remove_table' && isset($change[1]))
				wedbPackages::drop_table($change[1]);
			elseif ($change[0] == 'remove_column' && isset($change[2]))
				wedbPackages::remove_column($change[1], $change[2]);
			elseif ($change[0] == 'remove_index' && isset($change[2]))
				wedbPackages::remove_index($change[1], $change[2]);
		}
	}
...
function PackageOptions()
{
	...
	loadSubTemplate('install_options');
...
function PackagePermissions()
{
	...
	loadSubTemplate('file_permissions');
...
function PackagePermissionsAction()
{
	...
	loadSubTemplate('action_permissions');
