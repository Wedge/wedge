...
function aeva_loadAlbum($album_id = 0)
{
	...
	$context['aeva_album'] = array(
		...
		'description' => empty($album_info['description']) ? '' : parse_bbc($album_info['description'], 'media-album-description'),
		'owner' => array(
...
function aeva_loadCustomFields($id_media = null, $albums = array(), $custom = '')
{
	...
	while ($row = wesql::fetch_assoc($request))
	{
		$fields[$row['id_field']] = array(
			'id' => $row['id_field'],
			'name' => $row['name'],
			'desc' => empty($row['description']) ? '' : parse_bbc($row['description'], 'media-custom-field-description'),
			...
			'type' => $row['type'],
			'value' => !empty($row['value']) ? ($row['searchable'] ? aeva_getTags($row['bbc'] && !empty($row['value']) ? parse_bbc($row['value'], 'media-custom-field') : $row['value']) : ($row['bbc'] && !empty($row['value']) ? parse_bbc($row['value'], 'media-custom-field') : $row['value'])) : '',
...
function aeva_mkdir($dir, $chmod)
{
	...
	{
		loadSource(array('Subs-Package', 'Class-FTP')); // !!! Is Subs-Package needed here? Seems to me only the FTP connection is.
