...
function loadBBCodes() {
    ...
    // NOTE: As long as we didn't delete the default bbcodes from database we only
    // want to load bbcodes inserted by plugins. Otherwise we will have all default
    // bbcodes twice.
	$result = wesql::query('
		SELECT id_bbcode, tag, len, bbctype, before_code, after_code, content, disabled_before,
			...
			validate_func
		FROM {db_prefix}bbcode WHERE id_plugin != ""'
	);
