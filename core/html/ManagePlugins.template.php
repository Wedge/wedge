		...
		<p>';
			...
			if (!empty($plugin['readmes']))
			{
				...
				foreach ($plugin['readmes'] as $readme => $state)
					echo ' &nbsp;<a href="<URL>?action=admin;area=plugins;sa=readme;plugin=', rawurlencode($plugin['folder']), ';lang=', $readme, '" onclick="return reqWin(this);"><img src="', LANGUAGES, $context['languages'][$readme]['folder'], '/Flag.', $readme, '.gif"></a>';
			}
